/******/ (() => { // webpackBootstrap
/*!***********************************************!*\
  !*** ./src/js/modules/usk-grid-variations.js ***!
  \***********************************************/
/**
...
class USKGridVariations {
  ...
  setupEventListeners() {
    this.$container.on("click.usk-grid-variations", ".usk-variation-swatches__item", this.onSwatchSelect);
    this.$container.on("click.usk-grid-variations", ".usk-variation-button", this.onVariationButtonClick);
    this.$container.on("keydown.usk-grid-variations", ".usk-variation-swatches__item, .usk-variation-button", this.onKeyPress);
    this.$container.on("click.usk-grid-variations", ".usk-reset-variations", this.onResetClick);
    this.$container.on("click.usk-grid-variations", ".usk-back-variation", this.onBackClick);
  }
...
`);
      $resetButton.insertAfter(this.$container.find(".usk-variation-group").last());
      $resetButton.hide();
    }
  }
...
`);
      $backButton.insertAfter(this.$container.find(".usk-variation-group").first());
      $backButton.hide();
    }
  }
  ...
  onResetClick(event) {
    ...
    this.$container.find(".usk-variation-swatches__item, .usk-variation-button").removeClass("selected active disabled").data("disabled", false).attr("aria-pressed", "false").attr("tabindex", 0);
  ...
  resetAddToCartButton() {
    ...
    $addToCartBtn.removeClass("product_type_variation add_to_cart_button ajax_add_to_cart").addClass("product_type_variable").removeAttr("data-variation_id").attr("href", "javascript:void(0)");
    ...
    // Update button text
    const buttonText = $addToCartBtn.find(".usk-icon-arrow-right-8").length ? 'Select options <i class="button-icon usk-icon-arrow-right-8"></i>' : "Select options";
  ...
  selectDefaultAttributes() {
    ...
    for (const attrName in defaultAttributes) {
      if (defaultAttributes.hasOwnProperty(attrName)) {
        ...
        const $item = this.$container.find(`.usk-variation-swatches__item[data-value="${value}"]`);
        if ($item.length) {
          $item.trigger("click.usk-grid-variations");
        } else {
          // Try to find and select matching button
          const $button = this.$container.find(`.usk-variation-button[data-value="${value}"]`);
          if ($button.length) {
  ...
  onSwatchSelect(event) {
    ...
    // Update UI
    $wrapper.find(".usk-variation-swatches__item").removeClass("selected").attr("aria-pressed", "false");
  ...
  updateAvailableAttributes(isInitialLoad = false) {
    ...
    this.$container.find(".usk-variation-swatches__item, .usk-variation-button").removeClass("disabled").data("disabled", false).attr("tabindex", 0);
    ...
    if (isInitialLoad || !attributes.chosenCount) {
      ...
      this.$swatchWrappers.each((i, wrapper) => {
        ...
        $wrapper.find(".usk-variation-swatches__item").each((j, item) => {
          ...
          for (let v = 0; v < variations.length; v++) {
            ...
            if (!variation || !variation.attributes || !variation.is_in_stock || !variation.is_purchasable) {
              continue;
            }
            const variationAttrs = variation.attributes;

            // Check if this variation includes this attribute value
            if (variationAttrs[attributeName] === "" || variationAttrs[attributeName] === attributeValue) {
              isAvailable = true;
              break;
            }
          }
      ...
      this.$container.find(".usk-variation-group").each((i, group) => {
        ...
        $group.find(".usk-variation-button").each((j, button) => {
          ...
          // Disable if not available in any variation
          if (!isAvailable) {
            $button.addClass("disabled").data("disabled", true).attr("tabindex", -1);
          }
        });
      });
      return;
  ...
  resetAddToCartToSelectOptions($button) {
    $button.removeClass("product_type_variation add_to_cart_button ajax_add_to_cart").addClass("product_type_variable").removeAttr("data-variation_id").attr("href", "javascript:void(0)");

    // Clear attributes
    this.clearButtonAttributes($button);

    // Update text
    const buttonText = $button.find(".usk-icon-arrow-right-8").length ? 'Select options <i class="button-icon usk-icon-arrow-right-8"></i>' : "Select options";
  ...
  setUnavailableButton($button) {
    ...
    // Update text
    const buttonText = $button.find(".usk-icon-arrow-right-8").length ? 'Unavailable <i class="button-icon usk-icon-arrow-right-8"></i>' : "Unavailable";
    $button.html(buttonText);
  ...
  setAddToCartButton($button, variationId, attributes) {
    ...
    $button.removeClass("product_type_variable").addClass("product_type_variation add_to_cart_button ajax_add_to_cart").attr("data-product_id", this.productId).attr("data-variation_id", variationId).attr("data-prevent_redirect", "true").attr("href", "javascript:void(0)");
    ...
    // Update text
    const buttonText = $button.find(".usk-icon-arrow-right-8").length ? 'Add to cart <i class="button-icon usk-icon-arrow-right-8"></i>' : "Add to cart";
    $button.html(buttonText);
  }
...
jQuery(function ($) {
  ...
  $(document.body).on("wc_fragments_refreshed wc_fragments_loaded added_to_cart", initGridVariations);
...
/******/ })()
;
//# sourceMappingURL=usk-grid-variations.js.map