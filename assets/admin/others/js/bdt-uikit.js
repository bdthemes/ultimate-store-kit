...
/***/ "./src/admin/others/js/bdt-uikit.js"
/*!******************************************!*\
  !*** ./src/admin/others/js/bdt-uikit.js ***!
  \******************************************/
(module) {
...
(function (global, factory) {
  ...
  function animate$2(element, animation, duration = 200, origin, out) {
    return Promise.all(toNodes(element).map(element2 => new Promise((resolve, reject) => {
      ...
      const classes = [animation, clsAnimation, `${clsAnimation}-${out ? "leave" : "enter"}`, origin && `bdt-transform-origin-${origin}`, out && `${clsAnimation}-reverse`];
  ...
  function getCoveringElement(target) {
    ...
    for (const position of top ? [0, top] : [0]) {
      ...
      for (const el of toWindow(target).document.elementsFromPoint(left + width / 2, position)) {
        ...
        !hasClass(el, "bdt-togglable-leave") && (hasPosition(el, "fixed") && zIndex(parents(target).reverse().find(parent2 => !parent2.contains(el) && !hasPosition(parent2, "static"))) < zIndex(el) || hasPosition(el, "sticky") && parent(el).contains(target)) && (!coverEl || dimensions$1(coverEl).height < dimensions$1(el).height)) {
          coverEl = el;
        }
      }
      if (coverEl) {
  ...
  function createComponent(name, element, data, ...args) {
    ...
    }) : element ? findAll(element).map(init)[0] : init();
      ...
      updateNav() {
        ...
        for (const el of this.navItems) {
          ...
          if (isNumeric(cmd)) {
            ...
            toggleClass(el, "bdt-invisible", this.finite && (cmd === "previous" && index === 0 || cmd === "next" && index >= this.maxIndex));
          }
        }
      },
      padNavitems() {
  ...
  var notification = {
    ...
    created() {
      ...
      this.$mount(append(container, `<div class="${this.clsMsg}${this.status ? ` ${this.clsMsg}-${this.status}` : ""}" role="alert"> <a href class="${this.clsClose}" data-bdt-close></a> <div>${this.message}</div> </div>`));
    },
    async connected() {
  ...
  var Position = {
    ...
    methods: {
      ...
      getPositionOffset(element = this.$el) {
        return toPx(this.offset === false ? css(element, "--bdt-position-offset") : this.offset, this.axis === "x" ? "width" : "height", element) * (includes(["left", "top"], this.dir) ? -1 : 1) * (this.inset ? -1 : 1);
      },
      getShiftOffset(element = this.$el) {
        return this.align === "center" ? 0 : toPx(css(element, "--bdt-position-shift-offset"), this.axis === "y" ? "width" : "height", element) * (includes(["left", "top"], this.align) ? 1 : -1);
      },
  ...
  var tooltip = {
    ...
    methods: {
      ...
      async _show(title, id) {
        this.tooltip = append(this.container, `<div id="${id}" class="bdt-${this.$options.name}" role="tooltip"> <div class="bdt-${this.$options.name}-inner">${title}</div> </div>`);
  ...
  var Dropnav = {
    ...
    watch: {
      dropbar(dropbar) {
        addClass(dropbar, "bdt-dropbar", "bdt-dropbar-top", this.clsDropbar, `bdt-${this.$options.name}-dropbar`);
      },
      dropdowns() {
  ...
  var leader = {
    ...
    computed: {
      fill: ({
        fill
      }, $el) => fill || css($el, "--bdt-leader-fill-content")
    },
    connected() {
  ...
  function install({
    ...
    modal.alert = function (message, options) {
      return openDialog(({
        i18n
      }) => `<div class="bdt-modal-body">${isString(message) ? message : html(message)}</div> <div class="bdt-modal-footer bdt-text-right"> <button class="bdt-button bdt-button-primary bdt-modal-close" autofocus>${i18n.ok}</button> </div>`, options);
    };
    modal.confirm = function (message, options) {
      return openDialog(({
        i18n
      }) => `<form> <div class="bdt-modal-body">${isString(message) ? message : html(message)}</div> <div class="bdt-modal-footer bdt-text-right"> <button class="bdt-button bdt-button-default bdt-modal-close" type="button">${i18n.cancel}</button> <button class="bdt-button bdt-button-primary" autofocus>${i18n.ok}</button> </div> </form>`, options, () => Promise.reject());
    };
    modal.prompt = function (message, value, options) {
      const promise = openDialog(({
        i18n
      }) => `<form class="bdt-form-stacked"> <div class="bdt-modal-body"> <label>${isString(message) ? message : html(message)}</label> <input class="bdt-input" autofocus> </div> <div class="bdt-modal-footer bdt-text-right"> <button class="bdt-button bdt-button-default bdt-modal-close" type="button">${i18n.cancel}</button> <button class="bdt-button bdt-button-primary">${i18n.ok}</button> </div> </form>`, options, () => null, () => input.value);
      const {
        $el
      } = promise.dialog;
  ...
  var sticky = {
    ...
    update: [{
      read({
        ...
        sticky = maxScrollHeight && !this.showOnUp && start + offset$1 === topOffset && end === Math.min(maxScrollHeight, parseProp(true, this.$el, 0, true) - elHeight - offset$1 + overflow) && css(getVisibleParent(this.$el), "overflowY") === "visible";
        return {
          start,
          end,
          offset: offset$1,
...
/******/ 	var __webpack_exports__ = __webpack_require__("./src/admin/others/js/bdt-uikit.js");
/******/ 	
/******/ })()
;