/******/ (() => { // webpackBootstrap
/*!*******************************************************!*\
  !*** ./src/admin/others/js/usk-admin-api-biggopti.js ***!
  \*******************************************************/
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '.ultimate-store-kit-biggopti.is-dismissible .bdt-admin-api-biggopti-dismiss', function (e) {
    e.preventDefault();
    var $this = $(this).closest('.ultimate-store-kit-biggopti');
    ...
    var cfg = window.UltimateStoreKitBiggoptiConfig || window.UltimateStoreKitAdminApiBiggoptiConfig || {};
    ...
    $.ajax({
      ...
      type: 'POST',
      data: {
        action: 'usk_admin_api_biggopti_dismiss',
      ...
      $('.ultimate-store-kit-biggopti').filter(function () {
  ...
  function renderBiggoptiHTML(item) {
    ...
    var inner = '<div class="' + wrapperClass + '"' + (bg ? ' style="' + esc(bg) + '"' : '') + '>' + '<div class="bdt-api-biggopti-content">' + '<div class="bdt-plugin-logo-wrapper"><img height="auto" width="40" src="' + BIGGOPTI_ASSETS_URL + 'images/logo.svg" alt="Ultimate Store Kit Logo"></div>' + '<div class="bdt-biggopti-content">' + '<div class="bdt-biggopti-content-inner">' + logoHtml + '<div class="bdt-biggopti-title-description">' + (title ? '<h2 class="bdt-biggopti-title">' + title + '</h2>' : '') + (content ? '<div class="bdt-biggopti-html-content">' + content + '</div>' : '') + '</div></div>' + '<div class="bdt-biggopti-content-right">' + countdownHtml + btnHtml + '</div>' + '</div></div></div>';
    var endTs = endDate ? Math.max(new Date(endDate.replace(' ', 'T') + (tz === 'UTC' ? 'Z' : '')).getTime() - Date.now(), 0) : 604800;
    var classes = 'ultimate-store-kit-biggopti biggopti biggopti-info is-dismissible';
  ...
  function getCurrentVisibilitySectors() {
    ...
    if (page && (page === 'ultimate_store_kit_options' || page.toLowerCase().indexOf('ultimate_store_kit') !== -1)) {
  ...
  var FALLBACK = {
    sub_title: 'Go Pro',
    link: 'https://bdthemes.com/deals/?utm_source=WordPress_org&utm_medium=bfcm_cta&utm_campaign=ultimate_store_kit'
  };
  ...
  function fetchUskPromoData() {
    fetch(BIGGOPTI_API_URL).then(function (r) {
      return r.json();
    }).then(processApiData).catch(function () {
