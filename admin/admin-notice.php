...
class Notices {
	...
	private function get_api_notices_data() {
		...
		$transient_key = 'usk_api_notices_ultimate_store_kit';
		...
		if( isset($notices->api) && isset($notices->api->{'ultimate-store-kit'}) ) {
			$data = $notices->api->{'ultimate-store-kit'};
			if (is_array($data)) {
				$ttl = apply_filters('usk_api_notices_cache_ttl', 6 * HOUR_IN_SECONDS);
	...
	public function show_notices() {
		...
		foreach (self::$notices as $key => $notice) {
			...
			if (defined('BDTUSK_WL') && $notice['category'] === 'regular') {
				continue;
			}
